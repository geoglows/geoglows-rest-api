...
ACCOUNT=$(grep "accountName" /app/azcopy/file_mount.json -m 1 | awk -F':' '{print substr($2, 1, length($2)-1)}' | tr -d \")
OUTPUT_DIRECTORY="/mnt/output/ecmwf"
SIG=$(grep "sasSig" /app/azcopy/file_mount.json -m 1 | awk -F':' '{print substr($2, 1, length($2)-1)}' | tr -d \")

while read watershed
do
  if [[ $(find $OUTPUT_DIRECTORY/$watershed -maxdepth 1 -type d | wc -l) -eq 2 ]]
  then
    while read rawdate
    do
